...
const AdminBookings = () => {
  ...
  const getStatusBadge = (status) => {
    const defaultStyle =
      "px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 w-fit mx-auto";
    switch (status) {
      case "pending":
        return {
          icon: <Clock size={14} />,
          color: "bg-yellow-100 text-yellow-700 " + defaultStyle,
        };
      case "confirmed":
        return {
          icon: <CheckCircle2 size={14} />,
          color: "bg-primary/20 text-primary " + defaultStyle,
        };
      case "checked_in":
        return {
          icon: <CheckCircle2 size={14} />,
          color: "bg-blue-100 text-blue-700 " + defaultStyle,
        };
      case "cancellation_requested":
        return {
          icon: <AlertTriangle size={14} />,
          color: "bg-orange-100 text-orange-700 " + defaultStyle,
        };
      case "cancelled":
        return {
          icon: <Ban size={14} />,
          color: "bg-red-100 text-red-700 " + defaultStyle,
        };
      case "completed":
        return {
          icon: <CheckCircle2 size={14} />,
          color: "bg-green-100 text-green-700 " + defaultStyle,
        };
      default:
        return {
          icon: <Calendar size={14} />,
          color: "bg-gray-100 text-gray-500 " + defaultStyle,
        };
    }
  };
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-secondary">
          Manajemen & Approval Reservasi
        </h1>
        <p className="text-gray-500">
          Pantau semua pesanan masuk dan kelola status pembatalan.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 p-4 bg-white rounded-xl shadow-sm border border-gray-100">
        {[
          ...
          >
            ...
            {
              bookings.filter((b) => (s === "all" ? true : b.status === s))
                .length
            }
            )
          </button>
              ...
              {filteredBookings.length > 0 ? (
                        ...
                        <div className="flex gap-2 justify-center">
                          {b.status === "pending" && (
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleUpdateStatus(b._id, "confirmed", b.status)
                              }
                              disabled={isBusy}
                              className={`bg-primary hover:bg-primary-dark text-white px-3 py-1.5 rounded-lg text-xs font-medium active:scale-95 transition flex items-center gap-1 ${
                          ...
                          {b.status === "cancellation_requested" && (
                            ...
                            <>
                              <button
                                ...
                                className={`bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium active:scale-95 transition flex items-center gap-1 ${
                                  isBusy && "opacity-50 cursor-not-allowed"
                                }`}
                              >
                                {isBusy ? (
                              ...
                              <button
                                ...
                                className={`bg-gray-500 hover:bg-gray-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium active:scale-95 transition flex items-center gap-1 ${
                                  isBusy && "opacity-50 cursor-not-allowed"
                                }`}
                              >
                                {isBusy ? (
                          ...
                          {(b.status === "confirmed" ||
                            ...
                            <button
                              onClick={() =>
                                handleUpdateStatus(b._id, "completed", b.status)
                              }
                              disabled={isBusy}
                              className={`bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium active:scale-95 transition flex items-center gap-1 ${
                                isBusy && "opacity-50 cursor-not-allowed"
                              }`}
                            >
                              {isBusy ? (
                          ...
                          {(b.status === "cancelled" ||
                            b.status === "completed") && (
                            <span className="text-xs text-gray-500 bg-gray-100 p-2 rounded-lg">
                              Aksi Selesai
                            </span>
                          )}
                        </div>
                ...
                <tr>
                  <td colSpan="7" className="p-12 text-center text-gray-500">
                    Tidak ada reservasi di status `
                    {filter.toUpperCase().replace(/_/g, " ")}`.
                  </td>
                </tr>
              )}
            </tbody>
