...
const ManageUsers = () => {
  ...
  const handleDelete = async (id, name) => {
    if (
      window.confirm(
        `Yakin ingin menghapus pengguna "${name}"? Tindakan ini permanen.`
      )
    ) {
      try {
        const token =
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Kelola akun terdaftar dan hak akses admin.
          </p>
        </div>
                  ...
                  <tr key={u._id} className="hover:bg-gray-50 transition">
                    <td className="py-4 px-6 align-middle">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-lg uppercase shadow-sm
                          ${
                            u.role === "admin"
