...
import {
  ...
  Chart as ChartJS,
  Legend,
  LinearScale, // Pastikan BarElement terdaftar
  Title,
  Tooltip,
} from "chart.js";
import {
...
const DashboardHome = () => {
  ...
  const revenueLabels = data.charts.monthlyRevenue.map(
    (item) => monthNames[item._id - 1]
  );
  const revenueValues = data.charts.monthlyRevenue.map(
    (item) => item.total / 1000000
  ); // Dalam Juta
    ...
    datasets: [
      {
        ...
        data: activityLabels.map((m) =>
          getCountForMonth(data.charts.monthlyReservations, m)
        ),
      ...
      },
      {
        label: "Pembatalan",
        data: activityLabels.map((m) =>
          getCountForMonth(data.charts.monthlyCancellations, m)
        ),
      ...
      case "cancellation_requested":
        return (
          <span className="px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full flex items-center gap-1 w-fit">
            <AlertTriangle size={10} /> Cancel Req
          </span>
        );
      case "cancelled":
        return (
          <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-full w-fit">
            Cancelled
          </span>
        );
      default:
  ...
  const statsCards = [
    {
      ...
      color: "bg-blue-100",
      desc: "Booking baru masuk",
    },
    {
      title: "Batal Menunggu Acc",
      value: data.cards.pendingCancel,
      icon: <Clock className="text-yellow-600" />,
      color: "bg-yellow-100",
      desc: "Perlu tindakan admin",
    },
    {
      ...
      color: "bg-green-100",
      desc: "Akumulasi confirmed",
    },
    {
      title: "Total Properti",
      value: data.cards.totalSuites,
      icon: <BedDouble className="text-purple-600" />,
      color: "bg-purple-100",
      desc: "Unit terdaftar",
    },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Pantau performa bisnis dan aktivitas terbaru.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsCards.map((stat, index) => (
          <div
            key={index}
            className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex items-center gap-4 hover:shadow-xl transition-all hover:-translate-y-1 duration-300"
          >
            ...
            <div>
              ...
              <h3 className="text-xl md:text-2xl font-bold text-secondary mt-1">
                {stat.value}
              </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
          <h3 className="text-lg font-bold text-secondary mb-6 flex items-center gap-2">
            <BarChart3 size={20} className="text-blue-500" /> Tren Reservasi &
            Pembatalan
          </h3>
          <div className="h-64">
            <Bar
                ...
                {activeTab === "booking" ? (
                    ...
                    data.recentTransactions.map((trx) => (
                      <tr key={trx._id} className="hover:bg-gray-50 transition">
                        <td className="p-4 font-medium text-secondary flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-xs uppercase">
                  ...
                  data.recentCancellations.map((trx) => (
                    <tr key={trx._id} className="hover:bg-red-50 transition">
                      <td className="p-4 font-medium text-secondary flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-red-500 text-xs uppercase">
                          {trx.user?.name?.charAt(0) || "G"}
                        </div>
                        {trx.user?.name || "Guest"}
                      </td>
