...
// --- KOMPONEN TERPISAH UNTUK JAM REALTIME ---
...
const DashboardHome = () => {
  ...
  const getTypeColor = (type) => {
    ...
    switch (normalizedType) {
      case "Villa": return "#8B5CF6"; 
      case "Hotel": return "#3B82F6"; 
      case "Cottage": return "#F59E0B"; // Kuning
  ...
  const revenueLabels = activeRevenueData.map((item) => monthNames[item._id - 1]);
  ...
  const revenueOptions = {
    ...
    xAxis: {
      ...
      lineColor: "#e5e7eb",
      tickColor: "#e5e7eb",
      labels: { style: { color: "#6b7280", fontSize: "12px", fontWeight: "500" } },
    },
    yAxis: {
      title: { text: "" },
      gridLineColor: "#f3f4f6",
      labels: { formatter: function () { return "Rp " + (this.value / 1000000).toFixed(0) + "jt"; }, style: { color: "#6b7280", fontSize: "12px" } },
    },
    tooltip: {
      shared: true,
      useHTML: true,
      ...
      formatter: function () {
        return `<div style="min-width: 180px">
            <div style="color: #9ca3af; font-size: 11px; font-weight: 600; text-transform: uppercase; margin-bottom: 4px">${this.x}</div>
            <div style="color: #29D9D5; font-size: 18px; font-weight: 700">Rp ${new Intl.NumberFormat("id-ID").format(this.y)}</div>
    ...
    plotOptions: {
      areaspline: {
        fillColor: { linearGradient: { x1: 0, x2: 0, y1: 0, y2: 1 }, stops: [[0, "rgba(41, 217, 213, 0.3)"], [1, "rgba(41, 217, 213, 0)"]] },
        lineWidth: 3,
        color: "#29D9D5",
        marker: { enabled: true, radius: 4, states: { hover: { enabled: true, radius: 7 } } }, // Enable marker agar titik data terlihat jelas meski sedikit
      },
    },
  ...
  const activityOptions = {
    ...
    yAxis: { title: { text: "" }, gridLineColor: "#f3f4f6", labels: { style: { color: "#6b7280", fontSize: "12px" } } },
    tooltip: {
      shared: true,
      backgroundColor: "#fff",
      borderWidth: 0,
    ...
    plotOptions: { column: { borderRadius: 6, borderWidth: 0, groupPadding: 0.15, pointPadding: 0.05 } },
    series: [
      {
        name: "Booking",
        data: activeMonthIds.map((mId) => getCount(data.charts.monthlyReservations, mId)),
        color: { linearGradient: { x1: 0, x2: 0, y1: 0, y2: 1 }, stops: [[0, "#29D9D5"], [1, "#20B2AF"]] },
      },
      {
        name: "Pembatalan",
        data: activeMonthIds.map((mId) => getCount(data.charts.monthlyCancellations, mId)),
        color: { linearGradient: { x1: 0, x2: 0, y1: 0, y2: 1 }, stops: [[0, "#F87171"], [1, "#DC2626"]] },
      },
    ],
    legend: { align: "right", verticalAlign: "top", layout: "horizontal", itemStyle: { color: "#6b7280", fontWeight: "500", fontSize: "12px" }, itemHoverStyle: { color: "#29D9D5" } },
    credits: { enabled: false },
    exporting: { enabled: true, buttons: { contextButton: { enabled: false } } },
  };
    ...
    tooltip: {
      ...
      formatter: function () {
        return `<div style="padding: 4px">
          <div style="color: ${this.color}; font-weight: 700; font-size: 14px">${this.point.name}</div>
          <div style="color: #6b7280; font-size: 12px; margin-top: 2px">${this.y} unit (${this.percentage.toFixed(1)}%)</div>
        </div>`;
      },
    },
    credits: { enabled: false },
  ...
  const getStatusBadge = (status) => {
    const styles = {
      confirmed: "bg-gradient-to-r from-emerald-100 to-emerald-50 text-emerald-700 border-emerald-200",
      pending: "bg-gradient-to-r from-amber-100 to-amber-50 text-amber-700 border-amber-200",
      cancelled: "bg-gradient-to-r from-red-100 to-red-50 text-red-700 border-red-200",
      cancellation_requested: "bg-gradient-to-r from-orange-100 to-orange-50 text-orange-700 border-orange-200",
      checked_in: "bg-gradient-to-r from-blue-100 to-blue-50 text-blue-700 border-blue-200",
    };
    return (
      <span className={`px-3 py-1.5 rounded-lg text-xs font-bold border shadow-sm ${styles[status] || "bg-gray-100 text-gray-600"}`}>
        {status.replace(/_/g, " ").toUpperCase()}
      </span>
    );
  };

  if (loading) return <div className="flex h-[80vh] items-center justify-center"><Loader2 className="animate-spin text-primary w-12 h-12" /></div>;
    ...
    <div className="space-y-6 p-1">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight mb-1">Dashboard Overview</h1>
            <p className="text-gray-500 text-sm">Ringkasan performa bisnis Anda hari ini</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsCards.map((card, idx) => (
          <div key={idx} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 group">
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div className={`p-3 rounded-xl ${card.bg} shadow-sm group-hover:scale-110 transition-transform`}>
                  {card.icon}
                </div>
                {card.trend && (
                  <span className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${card.trendUp ? "text-green-600 bg-green-50 border border-green-200" : "text-orange-600 bg-orange-50 border border-orange-200"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                <Activity className="text-primary" size={20} /> Analisis Pendapatan
              </h3>
              <p className="text-sm text-gray-400 mt-1">Total pemasukan per bulan</p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-w-0">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-bold text-gray-800 text-lg">Distribusi Unit</h3>
              <p className="text-sm text-gray-400 mt-1">Per kategori properti</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-w-0">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-bold text-gray-800 text-lg">Traffic Pesanan</h3>
              <p className="text-sm text-gray-400 mt-1">Booking vs Pembatalan</p>
            </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-w-0 flex flex-col">
          <div className="flex border-b border-gray-100">
            <button
              ...
              className={`flex-1 py-4 text-sm font-semibold flex items-center justify-center gap-2 transition-all ${activeTab === "booking" ? "text-primary border-b-2 border-primary bg-cyan-50/30" : "text-gray-500 hover:bg-gray-50"}`}
            >
              <FileText size={16} /> Transaksi Terbaru
            </button>
            <button
              onClick={() => setActiveTab("cancellation")}
              className={`flex-1 py-4 text-sm font-semibold flex items-center justify-center gap-2 transition-all ${activeTab === "cancellation" ? "text-red-500 border-b-2 border-red-500 bg-red-50/30" : "text-gray-500 hover:bg-gray-50"}`}
            >
                ...
                {activeTab === "booking" ? (
                  data.recentTransactions.length > 0 ? (
                    data.recentTransactions.map((trx) => (
                      <tr key={trx._id} className="hover:bg-gray-50/80 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-semibold text-gray-700">{trx.user?.name || "Guest"}</span>
                          </div>
                        </td>
                        <td className="p-4 text-gray-600">{trx.suite?.name}</td>
                        <td className="p-4 text-gray-500 font-mono text-xs whitespace-nowrap">{new Date(trx.createdAt).toLocaleDateString("id-ID")}</td>
                        <td className="p-4">{getStatusBadge(trx.status)}</td>
                      </tr>
                    ))
                  ) : (
                    <tr><td colSpan="4" className="p-8 text-center text-gray-400 italic">Belum ada transaksi minggu ini.</td></tr>
                  )
                ) : data.recentCancellations.length > 0 ? (
                  data.recentCancellations.map((trx) => (
                    <tr key={trx._id} className="hover:bg-red-50/30 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-700">{trx.user?.name || "Guest"}</span>
                        </div>
                      </td>
                      <td className="p-4 text-gray-600">{trx.suite?.name}</td>
                      <td className="p-4 text-gray-500 font-mono text-xs whitespace-nowrap">{new Date(trx.updatedAt).toLocaleDateString("id-ID")}</td>
                      <td className="p-4">{getStatusBadge(trx.status)}</td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan="4" className="p-8 text-center text-gray-400 italic">Tidak ada pembatalan baru.</td></tr>
                )}
              </tbody>
            </table>
          </div>
