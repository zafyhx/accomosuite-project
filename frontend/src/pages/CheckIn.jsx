...
import { Upload, CheckCircle, Clock, User, FileText, ArrowLeft, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6">
      <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden">
        <div className="bg-primary p-6 text-white flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="bg-white/20 p-2 rounded-full hover:bg-white/30"><ArrowLeft size={20}/></button>
        ...
        <div className="p-8">
            <div className="mb-8 bg-blue-50 p-4 rounded-xl border border-blue-100">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                    <p>Properti: <span className="font-bold text-gray-800">{booking.suite?.name}</span></p>
                    <p>Total: <span className="font-bold text-primary">Rp {booking.totalPrice?.toLocaleString()}</span></p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Nomor Identitas (KTP/SIM/Paspor)</label>
                ...
                <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Estimasi Jam Kedatangan</label>
                    <div className="relative">
                ...
                <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Upload Foto Identitas</label>
                    <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center bg-gray-50 hover:bg-gray-100 transition">
                        {preview ? (
                            <div className="relative">
                                <img src={preview} alt="KTP Preview" className="h-40 mx-auto object-cover rounded-lg shadow-sm"/>
                                <button type="button" onClick={() => {setPreview(null); setFormData({...formData, idCardImage: null})}} className="text-red-500 text-sm mt-2 font-bold">Ganti Foto</button>
                            </div>
                        ) : (
