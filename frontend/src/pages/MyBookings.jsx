...
const MyBookings = () => {
  ...
  const fetchMyBookings = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 401) {
        ...
        setError(
          "Gagal memuat riwayat pesanan. Pastikan server backend berjalan."
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleCancelBooking = async (bookingId, suiteName) => {
    if (
      !window.confirm(
        `Anda yakin ingin MENGIRIM PENGAJUAN pembatalan untuk pesanan di ${suiteName}? Proses ini membutuhkan persetujuan Admin.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        `Pengajuan pembatalan untuk ${suiteName} berhasil dikirim. Menunggu persetujuan Admin.`
      );
  ...
  const getStatusBadge = (status) => {
    const defaultStyle =
      "px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border flex items-center gap-1 w-fit";
    switch (status) {
      case "confirmed":
        return {
          icon: <CheckCircle2 size={14} />,
          color: "bg-green-100 text-green-700 border-green-200 " + defaultStyle,
        };
      case "checked_in":
        return {
          icon: <CheckCircle2 size={14} />,
      ...
      case "cancelled":
        return {
          icon: <Ban size={14} />,
          color: "bg-red-100 text-red-700 border-red-200 " + defaultStyle,
        };
      case "cancellation_requested":
        return {
          icon: <AlertTriangle size={14} />,
          color:
            "bg-orange-100 text-orange-700 border-orange-200 " + defaultStyle,
        };
      default:
        return {
          icon: <Clock size={14} />,
          color:
            "bg-yellow-100 text-yellow-700 border-yellow-200 " + defaultStyle,
        }; // pending
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 pt-24 pb-20 font-sans">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <p className="text-gray-500 mb-8">
          Kelola dan pantau semua pesanan akomodasi Anda di sini.
        </p>
        ...
        {bookings.length === 0 ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl p-10 text-center border border-gray-100">
            ...
            <p className="text-gray-500 mb-6">
              Mulai jelajahi dan pesan tempat menginap impian Anda!
            </p>
            <Link
              to="/"
                      ...
                      <div>
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="text-xl font-bold text-secondary">
                            {item.suite?.name || "PROPERTI DIHAPUS"}
                          </h3>
                          <p className="text-xs text-gray-400 font-mono">
                            Booking ID: {item._id.substring(0, 10)}...
                          </p>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-gray-100 mt-2 gap-4">
                        ...
                        <div className="flex gap-3">
                          ...
                          {(item.status === "confirmed" ||
                            ...
                            <button
                              onClick={() =>
                                handleCancelBooking(item._id, item.suite?.name)
                              }
