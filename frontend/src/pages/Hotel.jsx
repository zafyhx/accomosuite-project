...
import { 
  MapPin, Star, Wifi, Coffee, ArrowRight, Search, 
  SlidersHorizontal, Filter, Loader2, CheckCircle2, LayoutGrid, 
  Crown, Home, Building2, Palmtree, Tent, TicketPercent // Ikon baru ditambahkan
} from 'lucide-react';
...
export default function HotelsPage() {
    ...
    useEffect(() => {
        ...
        if (activeCategory !== 'Semua') {
            ...
            result = result.filter(suite => 
                suite.type && suite.type.toLowerCase() === activeCategory.toLowerCase()
            );
        }
    ...
    // Top Recommendations
    const topRecommendations = [...suites].sort((a, b) => b.price - a.price).slice(0, 3);
    ...
    return (
        <div className="min-h-screen bg-gray-50 pt-24 pb-20 font-sans">
                        ...
                        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                            ...
                            <div className="flex-1 min-w-[200px] bg-gray-50 px-4 py-2 rounded-xl border border-gray-200">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Max Budget</span>
                                ...
                                <input 
                                    type="range" 
                                    min="0" max="15000000" step="500000"
                                    value={priceRange} 
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                ...
                {!loading && topRecommendations.length > 0 && searchTerm === '' && activeCategory === 'Semua' && (
                    <div className="mb-12 animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {topRecommendations.map((suite) => (
                                <Link to={`/suites/${suite._id}`} key={suite._id} className="group relative h-96 rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500">
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4 p-4 bg-white/90 backdrop-blur-xl rounded-2xl z-20 border border-white/40 shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h3 className="font-bold text-gray-900 text-lg line-clamp-1">{suite.name}</h3>
                                                <p className="text-xs text-gray-500 flex items-center gap-1 mt-1"><MapPin size={12}/> {suite.location}</p>
                                            </div>
                                        ...
                                        <div className="mt-3 pt-3 border-t border-gray-200/50 flex justify-between items-center">
                                            <p className="font-extrabold text-primary text-lg">Rp {suite.price?.toLocaleString()}</p>
                ...
                {!loading && (
                    <div className="relative bg-gradient-to-r from-gray-900 to-gray-800 rounded-3xl p-8 mb-12 overflow-hidden shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        <div className="relative z-10 text-white">
                            ...
                            <h3 className="text-3xl font-bold mb-2">Diskon 20% Untuk Member Baru!</h3>
                            <p className="text-gray-400 text-sm">Gunakan kode voucher <span className="text-white font-mono bg-white/20 px-2 py-1 rounded">ACCOMO20</span> saat checkout.</p>
                        </div>
                ...
                <div className="flex items-center justify-between mb-8">
                     <h3 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        ...
                        {activeCategory === 'Semua' ? 'Semua Penginapan' : `Kategori: ${activeCategory}`}
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32">
                        ...
                        <p className="text-gray-500 animate-pulse">Sedang mencari penginapan terbaik...</p>
                    </div>
                    ...
                    <div className="text-center py-20 bg-white rounded-[2rem] border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500 mt-2 mb-6 max-w-md mx-auto">
                            Kami tidak dapat menemukan penginapan dengan kriteria "{searchTerm}" di kategori {activeCategory}.
                        </p>
                        <button 
                            onClick={() => {setSearchTerm(''); setPriceRange(15000000); setActiveCategory('Semua');}}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {filteredSuites.map((suite) => (
                            <Link to={`/suites/${suite._id}`} key={suite._id} className="group bg-white rounded-3xl p-3 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col h-full">
                                {/* Image */}
                                ...
                                <div className="px-2 pb-2 flex flex-col flex-grow">
                                    <div className="mb-3">
                                        ...
                                        <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                                            <MapPin size={12} className="text-primary"/> {suite.location}
                                        </p>
                                    </div>
                                    ...
                                    <div className="mt-auto flex items-center justify-between border-t border-gray-100 pt-3">
                                        <div>
                                            <p className="text-[10px] text-gray-400 line-through">Rp {(suite.price * 1.2).toLocaleString()}</p>
